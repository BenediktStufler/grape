...
void *ballsinboxes(void *dim) {
	...
	while(1) {
		...
		while( sumE != m) {
			...
			// take next sample
			for(i=0, remN=n, sumE=0; i<n && remN != 0; i++) {
				if(i<max) {
					if(q[i]>0) {		// check if xi[i] > 0.0
						Nentry[i] = gsl_ran_binomial(rgen, q[i], remN);
						//DEBUG
		...
		// need to set remaining entries to zero;
		// contrarily to compiler warnings, we do not use i unitialized here
		for(;i<n;i++) Nentry[i]=0;
...
/*
 * Like ballsinboxes, but only for separable permutations
 */
void *ballsinboxes_sepa(void *dim) {
	// retrieve data that got passed to thread
	INT n = ((struct targ *)dim)->n;
	INT m = ((struct targ *)dim)->m;
	//mpfr_t *xi = ((struct targ *)dim)->xi;
	...
	while(1) {
		...
		while( sumE != m) {
			...
			for(i=0, remN=n, sumE=0; i<n && remN != 0; i++) {
				if(i) {
					Nentry[i] = gsl_ran_binomial(rgen, q[1], remN);
				} else
				{
					Nentry[i] = gsl_ran_binomial(rgen, q[0], remN);
				}	
				remN -= Nentry[i];
				sumE += i*Nentry[i];
...
/*
 * Like threadedbin, but only for separable permutations
 */
INT **threadedbinb_sepa(INT n, INT m, double *q, unsigned int max, unsigned int numThreads, gsl_rng **rgens, unsigned int num) {
	struct targ *argList;   // arguments for the separate threads
	pthread_t *th;			// array of threads
	INT i;
	void *ret;
	...
	*/
	...
	th = calloc(sizeof(pthread_t), numThreads);
	for(i=0; i<numThreads; i++) {
		if(pthread_create(&th[i], NULL, &ballsinboxes_sepa, &argList[i] )) {
			fprintf(stderr, "Error launching thread number %"STR(FINT)"\n", i);
			exit(-1);
		}
	}
...
/*
 * Like tbinb, but only for separable permutations
 */
INT *tbinb_sepa(INT n, INT m, double *q, unsigned int max, unsigned int numThreads, gsl_rng **rgens) {
	INT **multi;
	INT *single;
	multi = threadedbinb_sepa(n,m,q,max,numThreads,rgens,1);
	single = multi[0];
